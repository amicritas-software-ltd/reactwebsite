...
!(function(t, i) {
  ...
  function s(s, e) {
    ...
    this.items.addClass("colio-item"),
      (this.markup = t('<div class="colio"></div>')),
      this.markup.append('<div class="colio-container"></div>'),
      this.settings.navigation &&
        (this.markup.addClass("colio-has-navigation"),
        t('<div class="colio-navigation"></div>')
          .append(
            '<a class="colio-prev" href="#">' + this.settings.prevText + "</a>"
          )
          .append(
            '<a class="colio-next" href="#">' + this.settings.nextText + "</a>"
          )
          .appendTo(this.markup)),
      this.markup.append(
        '<a class="colio-close" href="#">' + this.settings.closeText + "</a>"
      ),
      "after" === this.settings.placement
        ? this.list.after(this.markup)
        : "inside" === this.settings.placement
        ? (this.markup.css({ position: "absolute", top: 0, left: 0 }),
          t("body").append(this.markup))
        : /^#\w/.test(this.settings.placement)
        ? (t(this.settings.placement).append(this.markup),
          (this.settings.placement = "element"))
        : (this.list.before(this.markup), (this.settings.placement = "before")),
      this.settings.id && this.markup.attr("id", this.settings.id),
      ...
      this.markup
        ...
        .click(function() {
          return (
            !o.lock &&
              o.position > 0 &&
              (o.position--, o.expand(o.items.eq(o.position))),
            !1
          );
        })
        .end()
        ...
        .click(function() {
          return (
            !o.lock &&
              o.position < o.items.length - 1 &&
              (o.position++, o.expand(o.items.eq(o.position))),
            !1
          );
        }),
      t(window).bind("resize orientationchange", function(t) {
        t.target === o.markup.get(0) && t.stopImmediatePropagation(),
          "inside" === o.settings.placement &&
            /^(smart)?resize$/.test(t.type) &&
            o.insideHideViewport(),
          o.temp || (o.temp = [o.settings.syncScroll, o.settings.scrollPage]),
          (o.settings.syncScroll = !0),
          (o.settings.scrollPage = !1),
          clearTimeout(o.resize_timer),
          (o.resize_timer = setTimeout(function() {
            o.expandViewport(o.active_id, !0),
              (o.settings.syncScroll = o.temp[0]),
              (o.settings.scrollPage = o.temp[1]),
              delete o.temp;
          }, 200));
      }),
      (this.api = {
        ...
        excludeHidden: function() {
          (o.items = o.list.children(":not(" + o.settings.hiddenItems + ")")),
            (o.position = o.items.index(o.list.find(".colio-active-item"))),
            "inside" === o.settings.placement && o.insideHideViewport();
        }
      }),
      this.list.data("colio", this.api),
      this.list.bind("colio", function(t, i, s) {
  ...
  (s.prototype.expand = function(t) {
    ...
    if (!this.lock && i) {
      (this.lock = !0),
        (this.position = this.items.index(t)),
        "inside" === this.settings.placement && this.insidePositionViewport(t);
      var s = t.data("colio-content-id");
      s
        ? this.expandViewport(s)
        : this.loadContent(i, function(i) {
            t.data("colio-content-id", i), this.expandViewport(i);
          }),
        this.markup
          .find(".colio-navigation a")
          .removeClass("colio-no-prev colio-no-next"),
        0 === this.position &&
    ...
    (s.prototype.loadContent = function(i, s) {
      var e = this,
        o = function(i) {
          ...
          return (
            t('<div id="' + o + '" class="colio-content"></div>')
              .append(i)
              .appendTo(e.markup.find(".colio-container")),
            "function" == typeof e.settings.onContent &&
              e.settings.onContent.call(
                e.markup.get(0),
                e.markup.find("#" + o).get(0)
              ),
            o
          );
        };
      if (/^#\w/.test(i)) {
        ...
        /#\w/.test(i) &&
          (this.settings.contentFilter = i.substr(i.indexOf("#")) + " " + l),
          this.items.eq(this.position).addClass("colio-item-loading"),
          t.get(i, function(t) {
            s.call(e, o(t)),
              (e.settings.contentFilter = l),
              e.items.eq(e.position).removeClass("colio-item-loading");
          });
      } else {
        var a = o(i);
        setTimeout(function() {
          ...
          this.markup.stop().animate(
            ...
            t.proxy(function() {
              this.markup.addClass("colio-expanded"),
                this.scroll(a, !n),
                (this.lock = !1),
                "function" == typeof this.settings.onExpand &&
                  this.settings.onExpand.call(this.markup.get(0), i);
            }, this)
          ),
          void this.scroll(a, n));
    }),
    (s.prototype.switchContent = function(i) {
      ...
      this.active_id !== i &&
        (s.length
          ? s.stop().fadeOut(
              this.settings.contentFadeOut,
              t.proxy(function() {
                e.fadeIn(this.settings.contentFadeIn);
              }, this)
            )
          : e
              .delay(this.settings.contentDelay)
              .fadeIn(this.settings.contentFadeIn),
        (this.active_id = i));
    }),
      ...
      return (
        o.is(":visible")
          ...
          : (o.css({
            ...
            (n = o.height()),
            o.css({ display: "", position: "", visibility: "", width: "" })),
        n > 0 ? n + e : 0
      );
    }),
    (s.prototype.scroll = function(i, s) {
      ...
      this.settings.scrollPage &&
        s &&
        t("body, html")
          .stop()
          .animate({ scrollTop: i }, e, o);
    }),
    (s.prototype.collapse = function() {
      ...
      (this.position = i),
        this.markup.stop().animate(
          ...
          t.proxy(function() {
            this.markup.find(".colio-content:visible").hide(),
              this.items.removeClass("colio-active-item"),
              this.scroll(this.page_scroll, !o),
              "function" == typeof this.settings.onCollapse &&
                this.settings.onCollapse.call(
                  this.markup.get(0),
                  this.active_id
                ),
              (this.active_id = i);
        ...
        "inside" === this.settings.placement && this.insideJoinGap(),
        this.scroll(this.page_scroll, o);
    }),
    (s.prototype.insideBottomItems = function(t) {
      ...
      return (
        this.settings.beforeLastRow ? (e = s - 1 > e ? e + 1 : e) : (e += 1),
        this.items.slice(e * i)
      );
    }),
    (s.prototype.insideMakeGap = function(s) {
      ...
      (a += parseFloat(this.markup.css("margin-bottom"))),
        t.each(this.insideBottomItems(s), function() {
          var s = t(this).data("colio-item-top");
          s === i &&
            ((s = parseFloat(t(this).css("top")) || 0),
            t(this).data("colio-item-top", s)),
            t(this).addClass("colio-bottom-item"),
            t(this)
              .stop()
              .animate({ top: s + a }, e, o);
        });
      ...
      l === i &&
        ((l = this.list.height()), this.list.data("colio-list-height", l)),
        this.list.height(l + a);
    }),
      ...
      this.items.filter(".colio-bottom-item").each(function() {
        ...
        i
          ...
          : t(this)
              ...
              .animate({ top: o }, s, e),
          t(this)
            .removeData("colio-item-top")
            .removeClass("colio-bottom-item");
      });
      var o = this.list.data("colio-list-height");
      this.list.addClass("no-transition").height(o),
        this.list.get(0).offsetHeight,
    ...
    (s.prototype.insidePositionViewport = function(t) {
      ...
      if (this.active_id) {
        ...
        s.length !== i.length &&
          (this.insideHideViewport(), this.insidePositionViewport(t));
      } else {
        /absolute|relative/.test(this.items.eq(0).css("position")) === !1 &&
        ...
        (n =
          i.length > 0
            ? i.offset().top
            : this.list.offset().top + this.list.height()),
          this.markup.css({ top: n, left: a, width: l });
      }
    }),
    (s.prototype.insideHideViewport = function() {
      this.active_id !== i &&
        (this.insideJoinGap(!0),
        this.markup
          .height(0)
          .find(".colio-content:visible")
          .hide(),
        this.items.removeClass("colio-active-item"),
        this.markup.removeClass("colio-expanded"),
        "function" == typeof this.settings.onCollapse &&
